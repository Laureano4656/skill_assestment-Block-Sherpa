...
const sampleProperties = [
	{
		...
		status: 'active',
		onChainPropertyId: 1 // Pre-seeded on smart contract
	},
	{
		...
		availability: 'available',
		description:
			'Perched atop a premier cast-iron landmark in Tribeca, this triplex penthouse offers 360-degree unobstructed vistas of Manhattan and the Hudson River. Includes a private wraparound rooftop terrace with outdoor kitchen, wood-burning hearth, private key-locked elevator entry, and custom Italian Poliform kitchen.',
		amenities: [
		...
		status: 'active',
		onChainPropertyId: 2 // Pre-seeded on smart contract
	},
	{
		...
		availability: 'available',
		description:
			'Striking minimalist desert modern retreat nestled against dramatic rock formations and ancient Joshua trees. Rammed-earth walls, passive solar heating, cantilevered shaded decks, saltwater plunge pool, and stargazing telescope platform offering total seclusion.',
		amenities: [
		...
		status: 'active',
		onChainPropertyId: null // Ready to register live on blockchain
	},
	{
		...
		availability: 'available',
		description:
			"Front-row Pacific oceanfront estate with private direct beach stairs. Open-concept entertaining pavilions with disappearing Fleetwood glass pocket doors, chef's dual kitchens, ocean-facing master suite with private balcony, and oceanfront hot tub.",
		amenities: [
	...
	},
	{
		...
		availability: 'available',
		description:
			'An extraordinary trophy property high above Sunset Strip offering world-class jetliner views from Downtown LA to the Pacific. Suspended infinity pool, imported terrazzo surfaces, private nightclub lounge, glass wine display, and elevator across all levels.',
		amenities: [
	...
	},
	{
		...
		availability: 'available',
		description:
			'Masterfully restored 25-foot wide Anglo-Italianate brownstone dating to 1860, combining historical crown mouldings and marble fireplaces with radiant underfloor heating, central HVAC, a landscaped English private garden, and rooftop deck overlooking lower Manhattan.',
		amenities: [
	...
	},
	{
		...
		availability: 'available',
		description:
			'Ultra-exclusive gated Star Island waterfront haven with 100 feet of deepwater dockage accommodating superyachts up to 130 feet. Features resort-style outdoor loggia, summer kitchen, spa sanctuary, double-height foyer, and quick boat access to the Atlantic.',
		amenities: [
	...
	},
	{
		...
		availability: 'available',
		description:
			'Ski-in/ski-out timber and stone mountain architectural triumph on Aspen Highlands. Vaulted heavy-timber ceilings, roaring Colorado fieldstone hearths, ski locker room with boot dryers, indoor lap pool, and panoramic snow-capped mountain views.',
		amenities: [
...
async function seedDatabase() {
	try {
		...
		console.log(`Removed ${deleted.deletedCount} old properties.`)

		// Insert new curated properties
		console.log(
			`Inserting ${sampleProperties.length} curated luxury properties...`
		)
		...
		created.forEach((p, idx) => {
			console.log(
				`#${idx + 1} [ID: ${p._id}] ${p.title} (${p.location}) - $${p.price.toLocaleString()} | On-Chain: ${p.onChainPropertyId ? '#' + p.onChainPropertyId : 'None (Ready to demo register)'}`
			)
		})
