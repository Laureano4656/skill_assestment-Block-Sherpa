...
const login = async (req, res) => {
	try {
		...
		console.log(
			`[Auth] Login attempt for email: ${email}, Remember Me: ${rememberMe}`
		)
		...
		const isLegacyUser =
			Registeruser.isEmailVerified === undefined &&
...
const forgotpassword = async (req, res) => {
	try {
		...
		try {
			...
			// Don't block the response — the token is already saved, the user can retry
		}
